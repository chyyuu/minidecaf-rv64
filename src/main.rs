...
pub mod generate_intermidiate_code {
    pub use crate::structs::*;

    pub fn generate_intermidiate_code(node: &Option<Node>) -> Vec<String> {
...
fn main() {
    ...
    let tokens = crate::lexing::lexing(&input);
    let node = crate::parsing::parsing(&tokens);
    let mid_commands = crate::generate_intermidiate_code::generate_intermidiate_code(&node);
    let native_commands = crate::generate_native_code::generate_native_code(&mid_commands);
